...
//
//  Created by Harry Dulaney on 11/18/19.
...
class LocationDemoViewController: UIViewController, CLLocationManagerDelegate {
    ...
    @objc func dismissKeyboard(){
          ...
          // Causes the view (or one of its embedded text fields) to resign their first responder status
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        if let location = locations.last {
            ...
            if Double(howRecent) < 15.0 {
                ...
                locationAccuracy.text = String(format: "%gm", location.horizontalAccuracy)
    ...
    func locationManager(_ manager: CLLocationManager, didFailWithError error: Error) {
        ...
        "Unknown Error"
        let alertController = UIAlertController(title: "Error Getting Location: \(errorType)", message: "Error Message: \(error.localizedDescription))",
    ...
    @IBAction func addressToCoordinantes(_ sender: Any) {
    ...
    private func processAddressResponse(withPlacemarks placemarks: [CLPlacemark]?,
                                        error: Error?) {
        if let error = error {
    ...
    @IBAction func deviceCoordinantes(_ sender: Any) {
